...
public class FletcherChecksum
{
    ...
    /// <param name="n"></param>
    /// <returns></returns>
    public static UInt64 GetChecksumFromString(string inputWord, int n)
    {
    ...
    /// <param name="n"></param>
    /// <returns></returns>
    public static UInt64 GetChecksumFromBytes(byte[] inputBytes, int n)
    {
        //Fletcher 16: Read a single byte
        //Fletcher 32: Read a 16 bit block (two bytes)
        //Fletcher 64: Read a 32 bit block (four bytes)
        ...
        UInt64 sum1 = 0;
        UInt64 sum2 = 0;
        foreach (UInt64 block in Blockify(inputBytes, bytesPerCycle))
        {
            sum1 = (sum1 + block) % modValue;
            sum2 = (sum2 + sum1) % modValue;
        }
